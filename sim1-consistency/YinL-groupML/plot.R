...
setwd(here::here("sim1-consistency/YinL"))
...
p.bayes.tmle <- plot.tmle(r'($\psi(\hat{Q}^*)$ - bayes)',ylim.var=c(170,250))

## bayes-onestep====
load("Onestep-bayes/result.Rdata")
p.bayes.one <- plot.tmle(r'($\psi^{+}(\hat{Q})$ - bayes)',ylim.var=c(170,250))
