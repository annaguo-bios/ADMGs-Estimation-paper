...
generate_data <- function(n,parA = c(0.48, 0.07, 1.00, -1.00, -0.34, -0.12, 0.30, -0.35, 1.00, -0.10, 0.46, # linear terms
  ...
  L <- parL[1] + parL[2]*M[,1] + parL[3]*M[,2] + parL[4]*X[,1] + parL[5]*U1 + rnorm(n,0,sd.L) # p(L|M,X,U1)

  Y <- parY[1] + parY[2]*L  + parY[3]*A + parY[4]*X[,1] + parY[5]*U2  + rnorm(n, 0, sd.Y) # p(Y|L,A,X,U2)
