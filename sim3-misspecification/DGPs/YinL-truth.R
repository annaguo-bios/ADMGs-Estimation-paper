...
density.l.max <- function(parL, l, m, a, x, sd.L) {
  dnorm(l, mean = parL[1] + parL[2] * a + parL[3] * m[1] + parL[4] * m[2] + parL[5] * x + parL[6]* a * x, sd.L)
}
...
compute_psi_and_var <- function(a, parA, parU, parM, parL, parY, sd.L, sd.M, n ) {
  ...
  mu.L.a0 <- parY[1] + parY[2] * (parL[1] + parL[2] * a0 + parL[3] * M[, 1] + parL[4] * M[, 2] + parL[5] * X + parL[6]* a0 * X) + # a0, replace L with E(L|A=a0,X)
