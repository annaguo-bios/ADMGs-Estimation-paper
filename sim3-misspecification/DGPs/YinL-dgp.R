...
generate_data <- function(n,parA = c(1,1), parU=c(1,1,1,1), parM = matrix(c(1, 1, 1, 1,-1,-0.5,2,-1), nrow = 2,byrow = T), parL= c(1,1,1, 1, 1,1) ,parY = c(1, 1, 1, 1, 1, 1), sd.U=1, sd.M= matrix(c(2, 1, 1, 3), nrow = 2), sd.L=1, sd.Y=1){
  ...
  L <- parL[1] + parL[2]*A + parL[3]*M[,1] + parL[4]*M[,2] + parL[5]*X + parL[6]*A*X + rnorm(n,0,sd.L) # p(L|A,X)
